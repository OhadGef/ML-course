...
num_sentences = 5
senten_min_length = 12
newSent = []
space = " "
myfile = open('outNewJokes.csv', 'w')
...
for i in range(num_sentences):
    ...
    while len(sent) < senten_min_length:
            ...
            newSent = space.join(sent) 
           
    myfile.write("%s" % newSent.encode("utf-8")+"\n")
...
# similarity between the new sentences
import re, math
from collections import Counter

file1 = open('data/LouisCK.csv', 'rb')
read = csv.reader(file1)
for row in read:
    data1 = list(read)


file2 = open('outNewJokes.csv', 'rb')
...
for idx,x in enumerate(data2):
    ...
    print 'cosine:',(idx+1), cosine/len(data1)
    cosine=0
